...
{
public:
	aligned_stream(STREAM &stream, unsigned align_increment, unsigned align_offset = 0, const std::function<void(char *, size_t)> &on_flush = nullptr, bool multiple = false)
		: _stream(stream), _buffer(align_increment, 0), _offset(0), _alignOffset(align_offset), _alignIncrement(align_increment), _onFlush(on_flush), _badChunk(false), _good(true), _multiple(multiple)
	...
	void flush()
	{
		if (_offset > 0)
		{
			if (_multiple)
			{
				std::string file_path = fmt::format("{}/offset_{}", "/template_data", std::to_string(_offset) + "_" + std::to_string(_totalCount));
				std::ofstream file(file_path, std::ios::binary);
				file.write(_buffer.data(), _buffer.size());
				file.close();
				printf("save template data as %s\n", file_path.c_str());
			}
			else
			{
