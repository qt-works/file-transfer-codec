...
public:
	...
	template <typename MAT, typename STREAM>
	unsigned decode_fountain(const MAT &img, STREAM &ostream, unsigned color_mode = 1, bool should_preprocess = false, int color_correction = 2, bool multiple = false);
...
inline Decoder::Decoder(int ecc_bytes, int color_bits, bool interleave)
	: _eccBytes(ecc_bytes >= 0 ? ecc_bytes : cimbar::Config::ecc_bytes()), _eccBlockSize(cimbar::Config::ecc_block_size()), _colorBits(color_bits >= 0 ? color_bits : cimbar::Config::color_bits()), _bitsPerOp(cimbar::Config::symbol_bits() + _colorBits), _interleaveBlocks(interleave ? cimbar::Config::interleave_blocks() : 0), _interleavePartitions(cimbar::Config::interleave_partitions()), _decoder(cimbar::Config::symbol_bits(), _colorBits, cimbar::Config::dark(), 0xFF)
{
}
...
inline unsigned Decoder::decode_fountain(const MAT &img, FOUNTAINSTREAM &ostream, unsigned color_mode, bool should_preprocess, int color_correction, bool multiple)
{
	CimbReader reader(img, _decoder, color_mode, should_preprocess, color_correction);
	...
	{
		null_stream devnull;
		aligned_stream aligner(devnull, chunk_size, 0, update_md_fun, multiple);
		return do_decode(reader, aligner, legacy_mode);
	}

	aligned_stream aligner(ostream, ostream.chunk_size(), 0, update_md_fun, multiple);
	return do_decode(reader, aligner, legacy_mode);
}
